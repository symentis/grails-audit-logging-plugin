...
When called, the event handlers have access to oldObj and newObj definitions that will allow you to take action on what has changed.
    ...
    private Integer determineTruncateLength(ctx) {
        ...
        if (configuredTruncateLength <= domainMaxSize){
            ...
            log.warn "Configured truncateLength ${configuredTruncateLength} exceeds the ${AuditLoggingConfigUtils.auditConfig.auditDomainClassName} oldValue/newValue 'maxSize' constraint. Truncating at maxSize: ${domainMaxSize}."
